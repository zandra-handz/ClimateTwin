...
def cache_and_push_notif_location_update(user_id, state, base_location, location_id, name, latitude, longitude, last_accessed):
...
# No push
def cache_notif_location_update(user_id, state, base_location, location_id, name, latitude, longitude, last_accessed):
    from django.core.cache import cache 
...
def cache_and_push_notif_new_gift(user_id, user_username, recipient_id, inbox_item_id, gift_request_id, gift_id):
    from django.core.cache import cache 
...
def cache_and_push_notif_friend_request(user_id, user_username, recipient_id, inbox_item_id, friend_request_id, friend_id):
    from django.core.cache import cache

    cache_key = f"last_notification_{recipient_id}"
    new_friend_request_message = f'{user_username} has sent you a friend request!'
    data = {'notification': new_friend_request_message, 'inbox_item_id': inbox_item_id, 'friend_request_id': friend_request_id, "friend_id": friend_id}
    cache.set(cache_key, data) #, timeout=3600) 
