function displayMapAnimation(mapContainerId, user_token) {
    ...
    function createDot(latitude, longitude, mapContainer) {
      ...
      if (
        x < 0 ||
        x > mapCanvas.offsetWidth ||
        y < 0 ||
        y > mapCanvas.offsetHeight
      ) {
    ...
    const socket = new WebSocket(
      `wss://climatetwin.com/ws/climate-twin/?user_token=${user_token}`
    );
  ...
  function displayMapUpdate(mapContainerId, user_token) {
    ...
    const socket = new WebSocket(
      `wss://climatetwin.com/ws/climate-twin/current/?user_token=${user_token}`
    );
    socket.onopen = function (event) {
